...
export async function GET(
  ...
  let res: Response;
  try {
    res = await backendFetch(`/api/v1/skills/${sport}`, { method: "GET" });
  } catch {
    return unavailable();
  }
  ...
  if (!res.ok) {
    ...
    return NextResponse.json(
      {
        title: data?.title ?? "Erro ao buscar habilidades",
...
export async function POST(
  ...
  let res: Response;
  try {
    res = await backendFetch(`/api/v1/users/me/skills/${sport}`, {
      method: "POST",
  ...
  if (!res.ok) {
    ...
    return NextResponse.json(
      {
        title: data?.title ?? "Erro ao salvar habilidades",
        type: data?.type ?? "server_error",
        message: data?.message ?? "Ocorreu um erro inesperado.",
      },
      { status: res.status }
